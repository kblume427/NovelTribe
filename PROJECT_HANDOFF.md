...
- **Anonymous Landing CTAs**: Signed-out visitors see `Start my private library` linking to sign-in and `See how it works` linking to onboarding; authenticated readers retain direct Tracker and Goodreads import actions.
...
- **Canonical Domain Enforcement (`https://novel-tribe.com`)**:
  ...
  - **Bug fix (mobile sign-in loop)**: Users who ever reached the raw `*.vercel.app` deployment URL (e.g. an old bookmark, home-screen PWA icon, or shared link) got Supabase auth cookies scoped to that origin, which are invisible to `novel-tribe.com`. This produced a "signs in via magic link, lands on Tracker, but Profile forces sign-in again" symptom whenever the two origins' cookie jars diverged (not Android-specific — any device/browser that had touched the `.vercel.app` origin could hit it). Added a code-level 308 redirect in `proxy.ts` that forces any request with a `*.vercel.app` host to `https://novel-tribe.com` before anything else runs, so a session can never be split across the two origins again.
